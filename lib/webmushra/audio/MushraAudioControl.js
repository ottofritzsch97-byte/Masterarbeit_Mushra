...
MushraAudioControl.prototype.getDuration = function() {
  return this.audioStimulus ? this.audioStimulus.getAudioBuffer().length : this.audioMaxPosition;
};
...
MushraAudioControl.prototype.process = function(audioProcessingEvent) {
  ...
  for (channel = 0; channel < outputBuffer.numberOfChannels; ++channel) {
    ...
    for (sample = 0; sample < outputBuffer.length; ++sample) {
      ...
      if (loopingActive && (currentPosition == (audioBuffer.length - this.audioFadingMaxPosition))) { // loop almost at end => fading is triggered
        fadingActive = 1;
      ...
      // Use the actual stimulus length instead of fixed audioLoopEnd
      var currentStimulusLength = audioBuffer.length;
      if (currentPosition >= currentStimulusLength) {
...
MushraAudioControl.prototype.setLoopEnd = function(_end) {
  var currentStimulusLength = this.audioStimulus ? this.audioStimulus.getAudioBuffer().length : this.audioMaxPosition;
  if (_end <= currentStimulusLength && _end > this.audioLoopStart && (_end-this.audioLoopStart) >= this.audioMinimumLoopDuration) {
...
MushraAudioControl.prototype.setLoop = function(_start, _end) {
  var changed = false;
  var currentStimulusLength = this.audioStimulus ? this.audioStimulus.getAudioBuffer().length : this.audioMaxPosition;
  if (_start >= 0 && _start < currentStimulusLength && (_end-_start) >= this.audioMinimumLoopDuration
  ...
  if (_end <= currentStimulusLength && _end > this.audioLoopStart && (_end-_start) >= this.audioMinimumLoopDuration
...
MushraAudioControl.prototype.getCurrentStimulusLength = function() {
  return this.audioStimulus ? this.audioStimulus.getAudioBuffer().length : this.audioMaxPosition;
};
